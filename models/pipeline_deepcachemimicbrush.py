...
def sample_from_quad_center(total_numbers, n_samples, center, pow=1.2):
    while pow > 1:
        ...
        start_val = - (abs(center) ** (1/pow)) 
        end_val = (abs(total_numbers - center) ** (1/pow))
        
        
        x_values = np.linspace(start_val, end_val, n_samples+1)
        
        
        indices_raw = np.sign(x_values) * (np.abs(x_values) ** pow)
        
        
        indices = [0] + [int(x + center) for x in np.unique(np.int32(indices_raw))[1:-1]]
    ...
    if pow <= 1:
        # 如果 pow 降到 1 还没找到合适的分布，直接退化为均匀分布（或者报错）
        # 这里为了不报错崩溃，可以考虑由你自己决定是报错还是 fallback
        # 原逻辑是报错：
        raise ValueError("Cannot find suitable pow. Please adjust n_samples or decrease center.")
...
class MimicBrushPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # interval_seq = [49, 47, 41, 33, 24, 15, 7, 0]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                cross_attention_kwargs["lpf_threshold"] = lpf_threshold
                cross_attention_kwargs["dyna_mask"] = mask
                cross_attention_kwargs["alpha"] = alpha
                cross_attention_kwargs["lpf_sigma"] = lpf_sigma
                cross_attention_kwargs["lpf_kernel"] = lpf_kernel
                cross_attention_kwargs["skip_boost_factor"] = skip_boost_factor
        ...
        # ====== VAE Decode 阶段 ======
        if not output_type == "latent":
            # 准备条件 kwargs（MimicBrush / Inpainting 所需）
            condition_kwargs = {}
            if isinstance(self.vae, AsymmetricAutoencoderKL):
                # 注意：你需要确保 init_image 和 mask_condition 在当前作用域可用
                # 通常它们在 pipeline.__call__ 的参数中，例如 self.init_image, self.mask_condition
                init_image = init_image.to(device=device, dtype=latents.dtype)
                init_image_condition = init_image.clone()
                mask_condition = mask_condition.to(device=device, dtype=latents.dtype)
                condition_kwargs = {"image": init_image_condition, "mask": mask_condition}

            # 核心：根据 output_all_sequence 决定 decode 方式
            if output_all_sequence:
                # latents_list: List[Tensor], 每个是 [B, C, H, W]
                image = [
                    self.vae.decode(
                ...
                num_img = image.shape[0]  # batch size

            # Safety checker（可选，MimicBrush 通常不需要）
            has_nsfw_concept = None  # 或保留原逻辑：self.run_safety_checker(...)

        else:
            # 直接返回 latent，也区分序列与否
            if output_all_sequence:
                image = latents_list
                num_img = len(latents_list)
            else:
                image = latents
        ...
        # ====== Postprocess 阶段 ======
        if has_nsfw_concept is None:
            # 注意：do_denormalize 是 per-image list
        ...
        # 统一后处理
        if output_all_sequence:
            # 对序列中的每一帧做 postprocess
            # 注意：do_denormalize 需要与每帧对应；这里简化为每帧都 True
            image = [
                self.image_processor.postprocess(
                    img, output_type=output_type, do_denormalize=[True]  # 每次只处理一个 img，所以用 [True]
                )
                for img in image
            ]
        else:
